...
public class PaySlip_Emp extends JFrame   implements ActionListener
...
{	
    ...
    PaySlip_Emp(int id) {
		...
		ch1.add(String.valueOf(id));
		ch2=new Choice();
		try {
			ConnectionClass obj=new ConnectionClass();
			String q="select Month from salary1";
			ResultSet rest=obj.stm.executeQuery(q);
			while(rest.next()) {
				ch2.add(rest.getString("Month"));
			}
		}
		catch(Exception ex) {
			ex.printStackTrace();
		...
		try {
			ConnectionClass obj=new ConnectionClass();
			String q="select year from salary1";
			ResultSet rest=obj.stm.executeQuery(q);
			while(rest.next()) {
				ch3.add(rest.getString("year"));
			}
		}
		catch(Exception ex) {
			ex.printStackTrace();
	...
	public void actionPerformed(ActionEvent ex) {
		if(ex.getSource()==bt1) {
			...
			try {
				...
				String year =ch3.getSelectedItem();
				String q1="select * from salary1 where empid='"+eid+"' and month='"+month+"' and year='"+year+"'";
				...
				while(rest1.next()) {
					...
					basic_salary=Float.parseFloat(rest1.getString("basic_salary"));
			    	

					ha=Float.parseFloat(rest1.getString("ha"));
					hra=Float.parseFloat(rest1.getString("hra"));
					pf=Float.parseFloat(rest1.getString("pf"));
