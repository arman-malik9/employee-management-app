	 ...
	 public void actionPerformed(ActionEvent ee) {
		 if(ee.getSource()==b1){
			 try {
				 ...
				 String q="select * from salary1 where empid='"+eid+"'";
				 ResultSet rs=obj.stm.executeQuery(q);
				 if(rs.next()) {
					 String p="select emptype from employeedata where empid= '" +eid+"'";			           
			            ResultSet x=obj.stm.executeQuery(p);
			            while(x.next()) {
			            	ty=x.getString("emptype");	            	
			            }			            
			            if(ty.equals("Permanent")||ty.equals("Admin"))
			            {
		 ...
		 try {
			 ...
			 if(rs.next()) {
				 String p="select emptype from employeedata where empid= '" +eid+"'";			           
		            ...
		            if(ty.equals("Permanent"))
		            {
				        Perma cl= new Perma(empid);
		            }
		            else {
		            	Tempe cl= new Tempe(empid);
		            }					 
			 }
